...
export class ApiClient {
  ...
  public async authenticateEnterprise(apiKey: string): Promise<UserInfo> {
    ...
    if (this.secretStorage && this.authToken) {
      try {
        await this.secretStorage.store("neurolint.authToken", this.authToken);
      } catch (error) {
  ...
  private isValidApiKey(apiKey: string): boolean {
    // Basic API key format validation - no logging for security
    if (!apiKey || typeof apiKey !== "string") return false;
    if (apiKey.length < 8 || apiKey.length > 256) return false;
    if (apiKey.includes(" ") || apiKey.includes("\n") || apiKey.includes("\t"))
      return false;
    // Check for suspicious patterns that could indicate injection
    if (/[<>\"'&]/.test(apiKey)) return false;
    return true;
  }
    ...
    try {
      this.abortControllers.forEach((controller) => {
        try {
          ...
          // Ignore abort errors - request might already be completed
        }
      });
      this.abortControllers.clear();
    } catch (error) {
      console.error("Error cancelling requests:", error);
      // Force clear the map even if some aborts failed
      this.abortControllers.clear();
    }
  }
  ...
  private logApiCall(config: any, status: number, data: any): void {
    ...
    const sanitizedConfig = {
      ...
      url: config.url,
      // Remove Authorization header and other sensitive data
    };
  ...
  private logApiError(error: any): void {
    ...
    const logEntry = {
      ...
      url: error.config?.url, // URL only, no sensitive headers
      user: this.userInfo?.email || "unknown",
      team: this.configManager.getTeamId(),
      // Do not log request data or headers that might contain sensitive info
    };
