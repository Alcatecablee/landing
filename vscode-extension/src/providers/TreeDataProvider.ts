  ...
  implements vscode.TreeDataProvider<NeuroLintTreeItem>
{
  ...
  private maxMemoryUsage: number = 10 * 1024 * 1024; // 10MB limit
  ...
  private getFileItems(): NeuroLintTreeItem[] {
    const fileItems: NeuroLintTreeItem[] = [];
  ...
  public updateAnalysisResults(filePath: string, result: any): void {
    ...
    const estimatedSize = JSON.stringify(resultData).length * 2; // rough estimate
    if (estimatedSize > this.maxMemoryUsage) {
      console.warn(
        `Analysis result too large (${estimatedSize} bytes), skipping cache`,
      );
      return;
    }

    this.analysisResults.set(filePath, resultData);

    // Manage cache size and memory bounds
    this.enforceMemoryLimits();
    this.refresh();
  }

  private enforceMemoryLimits(): void {
    // Remove oldest entries if over size or memory limits
    while (this.analysisResults.size > this.maxCacheSize) {
      const entries = Array.from(this.analysisResults.entries());
      entries.sort((a, b) => (a[1].timestamp || 0) - (b[1].timestamp || 0));
      const firstEntry = entries[0];
      if (firstEntry) {
        this.analysisResults.delete(firstEntry[0]);
      } else {
        break;
      }
    }

    // Rough memory estimation and cleanup
    const totalEntries = this.analysisResults.size;
    if (totalEntries > this.maxCacheSize * 0.8) {
      ...
      for (let i = 0; i < entriesToRemove && i < entries.length; i++) {
        ...
        if (entry) {
          this.analysisResults.delete(entry[0]);
        }
      }
    }
  }
