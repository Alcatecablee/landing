...
export class NeuroLintDiagnosticProvider {
  ...
  private async performAnalysis(document: vscode.TextDocument): Promise<void> {
    ...
    // Cancel existing analysis for this document to prevent race conditions
    const existingAnalysis = this.analysisQueue.get(uri);
    if (existingAnalysis) {
      // Wait for existing analysis to complete, then proceed with new one
      try {
        await existingAnalysis;
      } catch (error) {
        // Continue with new analysis even if previous failed
      }
    }
