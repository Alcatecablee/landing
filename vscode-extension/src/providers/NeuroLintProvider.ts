...
export class NeuroLintProvider {
  ...
  private setCachedResult(uri: string, result: any): void {
    ...
    // Basic bounds checking to prevent memory overflow
    const maxResultSize = 1024 * 1024; // 1MB per result
    const resultString = JSON.stringify(result);
    if (resultString.length > maxResultSize) {
      console.warn(
        `Analysis result too large (${resultString.length} chars), not caching`,
      );
      return;
    }
