...
async function analyzeFile(uri?: vscode.Uri): Promise<void> {
  try {
    ...
    // Critical error boundary - prevent extension crash
    const errorMessage =
      criticalError instanceof Error ? criticalError.message : "Critical error";
    outputChannel.appendLine(`CRITICAL ERROR in analyzeFile: ${errorMessage}`);
    statusBar.showError("Critical error occurred");
    console.error("NeuroLint critical error:", criticalError);
  }
}
...
export function deactivate() {
  try {
    ...
    if (configManager) {
      ...
      outputChannel?.appendLine("Configuration manager disposed");
    }
    ...
    // Emergency cleanup - dispose everything without error checking
    try {
      apiClient?.cancelAllRequests();
      neurolintProvider?.dispose();
      diagnosticProvider?.dispose();
