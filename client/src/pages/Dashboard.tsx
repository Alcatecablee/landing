...
export default function Dashboard() {
  ...
  const fetchUsageStats = async () => {
    ...
    try {
      // Simple API calls without aggressive timeouts
      let healthResponse;
      try {
        healthResponse = await safeFetch("/api/health", {
          method: "GET",
        });
      } catch (error) {
        // Skip health check if it fails
        console.debug("Health check skipped");
      ...
      let response;
      try {
        response = await safeFetch("/api/auth/usage", {
          headers: {
            Authorization: `Bearer ${user.id}`,
          },
        });
      } catch (error) {
        // Use fallback if usage API fails
        console.debug("Usage API unavailable, using fallback");
      ...
      console.debug("Usage stats unavailable, using fallback data");

      // Set fallback usage data based on user metadata
      if (user?.app_metadata) {
        setUsage({
          ...
          planType: user.app_metadata.plan_type || "free",
        });
      }
    } finally {
      setLoading(false);
        ...
        <div className="container mx-auto px-6 py-8 max-w-6xl">
          ...
          <div className="mb-8">
            ...
            <p className="text-zinc-400">
              Monitor your activity, manage settings, and track your progress
            </p>
          </div>
          <Tabs
            value={activeTab}
            ...
            className="w-full"
          >
            <TabsList className="grid w-full grid-cols-4 bg-zinc-900/50 border border-zinc-800/50 backdrop-blur-sm p-1 mb-8">
              <TabsTrigger
                value="overview"
                className="data-[state=active]:bg-white data-[state=active]:text-black text-zinc-400 px-4 py-2"
              >
